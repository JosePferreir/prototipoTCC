...
function BookProgress() {
  ...
  const registerRead = async () => {
    ...
    console.log(user)
    await fetch(`https://prototipo-tcc-fake-api.vercel.app/user_books/${newUserBook.id}`, {
      method: 'PUT',
      headers: {
    ...
    console.log("NOVO: user",newUser)
    await fetch(`https://prototipo-tcc-fake-api.vercel.app/user/${newUser.id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  const checkAndAddAchievements = async (newUser: User, genre: string) => {
    ...
    const userInsigniasResponse = await fetch(`https://prototipo-tcc-fake-api.vercel.app/user_insignias?usuarioId=${newUser.id}`);
    ...
    .forEach(async (insignia: Insignia) => {
      ...
      if (remainingBooks === 0) {
        await fetch('https://prototipo-tcc-fake-api.vercel.app/user_insignias', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
